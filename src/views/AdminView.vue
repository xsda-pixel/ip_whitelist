      ...
      render(row) {
        return h(NSwitch, {
          ...
          'onUpdate:value': async (newValue: boolean) => {
            ...
            updateUserStatusAPI(row.userId, newStatus)
              .then(() => {
...
const onAddUser = () => {
  ...
  if (typeof (plainCopy.ports as any) === 'string') {
    plainCopy.ports = (plainCopy.ports as string).split(',').map(Number).filter(Boolean) // 过滤掉转换出的 0 (比如空字符串或多余的逗号)
  }

  if (reqData.userId) {
    updateUserAPI(plainCopy)
      .then(() => {
...
<template>
  <main class="flex h-screen flex-col items-center justify-center gap-4 pt-[5vh]">
    <div class="flex w-full justify-end gap-4 pr-10">
      <div class="flex items-center gap-3 text-sm">
        <n-button @click="showModal = true" type="primary">添加</n-button>
  ...
  <n-modal :mask-closable="false" preset="dialog" v-model:show="showModal" @close="onCloseModal">
    <template #header>
      <div>添加账号</div>
    </template>
    <div class="my-4 flex flex-col gap-4">
      <n-select
        v-model:value="reqData.provider"
        :disabled="reqData.userId"
        placeholder="请选择云服务商"
        :options="providerOptions"
      />
      <n-input
        v-model:value.trim="reqData.region"
        autocapitalize="off"
        autocorrect="off"
        spellcheck="false"
        autocomplete="off"
      ...
      />
      <n-input
        v-model:value.trim="reqData.accessKey"
        autocapitalize="off"
        autocorrect="off"
        spellcheck="false"
        autocomplete="off"
      ...
      />
      <n-input
        v-model:value.trim="reqData.secretKey"
        autocapitalize="off"
        autocorrect="off"
        spellcheck="false"
        autocomplete="off"
      ...
      />
      <n-input
        v-model:value.trim="reqData.groupId"
        autocapitalize="off"
        autocorrect="off"
        spellcheck="false"
        autocomplete="off"
        type="text"
        clearable
        placeholder="请输入安全组ID(groupId)"
      />
      <n-input
        v-model:value.trim="reqData.ports"
        autocapitalize="off"
        autocorrect="off"
        spellcheck="false"
        autocomplete="off"
      ...
      />
      <n-input
        v-model:value.trim="reqData.remark"
        autocapitalize="off"
        autocorrect="off"
        spellcheck="false"
        autocomplete="off"
    ...
    <template #action>
      <div class="flex gap-4">
        <n-button size="small" @click="onCloseModal" type="tertiary">取消</n-button>
        <n-button size="small" @click="onAddUser" type="primary">确认</n-button>
      </div>
    </template>
  </n-modal>
</template>
